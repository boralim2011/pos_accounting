...
<section class="content-header">
    ...
    <li class="active">Manage Item_class</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <!-- <div class="box-header">
                   <h3 class="box-title">List of Item_class</h3>
                </div>-->
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-item_class"><i class="fa fa-plus"></i> Add</a>
                            <a href="#item_class" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="item_class_name" <?php echo isset($search_by) && $search_by=="item_class_name" ? 'selected="selected"':'';?> >Item_class Name</option>
                                        <option value="item_class_name_kh" <?php echo isset($search_by) && $search_by=="item_class_name_kh" ? 'selected="selected"':'';?> >Item_class Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="item_class-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Item_class Name </th>
                            <th>Name KH</th>
                            <th>Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(isset($item_classs) && is_array($item_classs)){
                            foreach($item_classs as $row){
                                ?>
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->item_class_name)? $row->item_class_name:"";?></td>
                                    <td><?php echo isset($row->item_class_name_kh)? $row->item_class_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>item_class/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#item_class-table').DataTable( {
            "scrollX": true,
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>item_class/manage_item_class";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-item_class .modal-title").html("View Item_class");
            $("#item_class-form").attr('action', '<?php echo base_url()?>item_class/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#item_class_id").val(model.item_class_id);
            $("#item_class_name").val(model.item_class_name);
            $("#item_class_name_kh").val(model.item_class_name_kh);

            $("#btn-add-item_class").remove();
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-item_class .modal-title").html("Edit Item_class");
            $("#item_class-form").attr('action', '<?php echo base_url()?>item_class/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('item_class_id', model.item_class_id);


                $.ajax({
                    //url: '<?php //echo base_url();?>item_class/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
