...
<section class="content-header">
    ...
    <li class="active">Manage Age_range</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <!-- <div class="box-header">
                   <h3 class="box-title">List of Age_range</h3>
                </div>-->
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-age_range"><i class="fa fa-plus"></i> Add</a>
                            <a href="#age_range" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="age_range_name" <?php echo isset($search_by) && $search_by=="age_range_name" ? 'selected="selected"':'';?> >Age_range Name</option>
                                        <option value="age_range_name_kh" <?php echo isset($search_by) && $search_by=="age_range_name_kh" ? 'selected="selected"':'';?> >Age_range Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="age_range-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                        ...
                        <tbody>
                        <?php if(isset($age_ranges) && is_array($age_ranges)){
                            foreach($age_ranges as $row){
                                ?>
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->age_range_name)? $row->age_range_name:"";?></td>
                                    <td><?php echo isset($row->age_range_name_kh)? $row->age_range_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>age_range/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#age_range-table').DataTable( {
            "scrollX": true,
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>age_range/manage_age_range";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-age_range .modal-title").html("View Age_range");
            $("#age_range-form").attr('action', '<?php echo base_url()?>age_range/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#age_range_id").val(model.age_range_id);
            $("#age_range_name").val(model.age_range_name);
            $("#age_range_name_kh").val(model.age_range_name_kh);

            $("#btn-add-age_range").remove();
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-age_range .modal-title").html("Edit Age_range");
            $("#age_range-form").attr('action', '<?php echo base_url()?>age_range/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('age_range_id', model.age_range_id);


                $.ajax({
                    //url: '<?php //echo base_url();?>age_range/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
