...
<section class="content-header">
    <h1>
        <?php echo isset($title)? $title:'New Mixed'; ?>
        <!--<small>Optional description</small>-->
    </h1>
    <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-home"></i> home</a></li>
    <li><a href="#item"><i class="fa fa-barcode"></i> Manage Item</a></li>
    <li class="active"><?php echo isset($title)? $title:'New Mixed'; ?></li>
    </ol>
</section>
<!-- Main content -->
...
<!--                    </div>-->
                    ...
                    <div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12" >
                        ...
                        <div id="item-members">
                            <?php $this->load->view('item/item_members', array());?>
                        </div>

                    </div>

<!--                    <div class="form-group col-xs-12 col-sm-12 col-md-12 col-lg-12" >-->
<!--                        <label for="unit_id">Item Members</label>-->
<!--                        -->
<!--                    </div>-->
...
//        });
        ...
        $(document).on("click",".btn-choose-item", function(event){
            ...
            var url = "<?php echo base_url()?>item/choose";

            var posting = $.post(
                url,
                { submit: 1, item_model : model, item_members:item_members, delete_item_members:delete_item_members },
                function (data, status, xhr) {
                    if (data == 521) {
                        go_to_login();
                    }
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        $(document).on("click","#submit", function( event )
        {
            ...
            var item_members = null;
            if(typeof items != typeof undefined && items!='' ) item_members = JSON.parse(items);
            formData.append('item_members', JSON.stringify(item_members));

            var delete_items = $("#item-members-table").attr('delete-json');
            var delete_item_members = null;
            if(typeof delete_items != typeof undefined && delete_items!='' ) delete_item_members = JSON.parse(delete_items);
            formData.append('delete_item_members', JSON.stringify(delete_item_members));
                ...
                success: function(data, status, xhr)
                {
                    ...
                    else
                    {
                        if(data.success===true){
                            ...
                            $("#item-members-table").attr('data-json', JSON.stringify(data.models));
                            $("#item-members-table").attr('delete-json', "");
                        }
                        //alert(data.message);
