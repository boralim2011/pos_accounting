...
<section class="content-header">
    ...
    <li class="active">Manage Room_type</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <!-- <div class="box-header">
                   <h3 class="box-title">List of Room_type</h3>
                </div>-->
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-room_type"><i class="fa fa-plus"></i> Add</a>
                            <a href="#room_type" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="room_type_name" <?php echo isset($search_by) && $search_by=="room_type_name" ? 'selected="selected"':'';?> >Room_type Name</option>
                                        <option value="room_type_name_kh" <?php echo isset($search_by) && $search_by=="room_type_name_kh" ? 'selected="selected"':'';?> >Room_type Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="room_type-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                        ...
                        <?php if(isset($room_types) && is_array($room_types)){
                            foreach($room_types as $row){
                                ...
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->room_type_name)? $row->room_type_name:"";?></td>
                                    <td><?php echo isset($row->room_type_name_kh)? $row->room_type_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>room_type/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#room_type-table').DataTable( {
            "scrollX": true,
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>room_type/manage_room_type";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-room_type .modal-title").html("View Room_type");
            $("#room_type-form").attr('action', '<?php echo base_url()?>room_type/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#room_type_id").val(model.room_type_id);
            $("#room_type_name").val(model.room_type_name);
            $("#room_type_name_kh").val(model.room_type_name_kh);
            //$("#manage_stock").iCheck(model.manage_stock==1?'check':'uncheck');
            //$('#parent_id').select2("val", model.parent_id);

            $("#btn-add-room_type").remove();
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-room_type .modal-title").html("Edit Room_type");
            $("#room_type-form").attr('action', '<?php echo base_url()?>room_type/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('room_type_id', model.room_type_id);
...
<?php $this->load->view('room_type/new_room_type'); ?>
