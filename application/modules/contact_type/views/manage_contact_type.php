...
<section class="content-header">
    ...
    <li class="active">Manage Contact_type</li>
    </ol>
</section>
<!-- Main content -->
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <!-- <div class="box-header">
                   <h3 class="box-title">List of Contact_type</h3>
                </div>-->
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-contact_type"><i class="fa fa-plus"></i> Add</a>
                            <a href="#contact_type" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="contact_type_name" <?php echo isset($search_by) && $search_by=="contact_type_name" ? 'selected="selected"':'';?> >Contact_type Name</option>
                                        <option value="contact_type_name_kh" <?php echo isset($search_by) && $search_by=="contact_type_name_kh" ? 'selected="selected"':'';?> >Contact_type Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="contact_type-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Contact_type Name </th>
                            <th>Name KH</th>
                            <th>Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(isset($contact_types) && is_array($contact_types)){
                            foreach($contact_types as $row){
                                ?>
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->contact_type_name)? $row->contact_type_name:"";?></td>
                                    <td><?php echo isset($row->contact_type_name_kh)? $row->contact_type_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>contact_type/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var t = $('#contact_type-table').DataTable( {
            "scrollX": true,
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>contact_type/manage_contact_type";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-contact_type .modal-title").html("View Contact_type");
            $("#contact_type-form").attr('action', '<?php echo base_url()?>contact_type/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#contact_type_id").val(model.contact_type_id);
            $("#contact_type_name").val(model.contact_type_name);
            $("#contact_type_name_kh").val(model.contact_type_name_kh);
            //$("#manage_stock").iCheck(model.manage_stock==1?'check':'uncheck');
            $('#parent_id').select2("val", model.parent_id);

            $("#btn-add-contact_type").remove();
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-contact_type .modal-title").html("Edit Contact_type");
            $("#contact_type-form").attr('action', '<?php echo base_url()?>contact_type/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('contact_type_id', model.contact_type_id);


                $.ajax({
                    //url: '<?php //echo base_url();?>contact_type/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
...
<?php $this->load->view('contact_type/new_contact_type'); ?>
