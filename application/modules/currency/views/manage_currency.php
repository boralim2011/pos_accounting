        ...
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <!-- <div class="box-header">
                   <h3 class="box-title">List of Currency</h3>
                </div>-->
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-currency"><i class="fa fa-plus"></i> Add</a>
                            <a href="#currency" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="currency_name" <?php echo isset($search_by) && $search_by=="currency_name" ? 'selected="selected"':'';?> >Currency Name</option>
                                        <option value="currency_name_kh" <?php echo isset($search_by) && $search_by=="currency_name_kh" ? 'selected="selected"':'';?> >Currency Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="currency-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Currency Name </th>
                        ...
                        <tbody>
                        <?php if(isset($currencys) && is_array($currencys)){
                            foreach($currencys as $row){
                                ?>
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->currency_name)? $row->currency_name:"";?></td>
                                    <td><?php echo isset($row->currency_name_kh)? $row->currency_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>currency/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>currency/manage_currency";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-currency .modal-title").html("View Currency");
            $("#currency-form").attr('action', '<?php echo base_url()?>currency/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#currency_id").val(model.currency_id);
            $("#currency_name").val(model.currency_name);
            $("#currency_name_kh").val(model.currency_name_kh);
            $("#currency_symbol").val(model.currency_symbol);

            $("#btn-add-currency").remove();
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-currency .modal-title").html("Edit Currency");
            $("#currency-form").attr('action', '<?php echo base_url()?>currency/edit' );
...
<?php $this->load->view('currency/new_currency'); ?>
