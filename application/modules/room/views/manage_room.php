    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-room"><i class="fa fa-plus"></i> Add</a>
                            <a href="#room" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="room_name" <?php echo isset($search_by) && $search_by=="room_name" ? 'selected="selected"':'';?> >Room Name</option>
                                        <option value="room_name_kh" <?php echo isset($search_by) && $search_by=="room_name_kh" ? 'selected="selected"':'';?> >Room Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="room-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Room Name </th>
                        ...
                        <?php if(isset($rooms) && is_array($rooms)){
                            foreach($rooms as $row){
                                ...
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->room_name)? $row->room_name:"";?></td>
                                    <td><?php echo isset($row->room_name_kh)? $row->room_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>room/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>room/manage_room";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-room .modal-title").html("View Room");
            $("#room-form").attr('action', '<?php echo base_url()?>room/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#room_id").val(model.room_id);
            $("#room_name").val(model.room_name);
            $("#room_name_kh").val(model.room_name_kh);
            //$('#room_type_id').select2("val", model.room_type_id);
            $('#room_type_id').empty().append('<option></option><option value="'+ model.room_type_id +'" selected="selected">'+ model.room_type_name +'</option>');
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-room .modal-title").html("Edit Room");
            $("#room-form").attr('action', '<?php echo base_url()?>room/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                $.ajax({
                    //url: '<?php //echo base_url();?>room/delete',
                    url: btn.attr("url"),
                    type: 'POST',
                    data: formData,
                    dataType:"json",
