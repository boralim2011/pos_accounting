    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-info">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;" >
                            <a href="#" class="btn btn-primary" id="btn-new-unit"><i class="fa fa-plus"></i> Add</a>
                            <a href="#unit" class="btn btn-primary btn-refresh"><i class="fa fa-refresh"></i> Refresh</a>
                        </div>
                        <div class="col-lg-6 col-sm-6 col-xs-12" style="margin-top: 3px;">
                            <div class="row">
                                <div class="form-group ccol-lg-6 col-sm-6 col-xs-12">
                                    <select id="search_by" name="search_by" class="form-control select2" data-placeholder="Search By"  style="width: 100%;">
                                        <option value="unit_name" <?php echo isset($search_by) && $search_by=="unit_name" ? 'selected="selected"':'';?> >Unit Name</option>
                                        <option value="unit_name_kh" <?php echo isset($search_by) && $search_by=="unit_name_kh" ? 'selected="selected"':'';?> >Unit Name KH</option>
                                    </select>
                                </div>
                ...
                <div class="box-body">
                    <table id="unit-table" class="table table-bordered table-hover data-table">
                        <thead>
                        <tr>
                            <th>No</th>
                            <th>Unit Name </th>
                        ...
                        <?php if(isset($units) && is_array($units)){
                            foreach($units as $row){
                                ...
                                <tr>
                                    <td></td>
                                    <td><?php echo isset($row->unit_name)? $row->unit_name:"";?></td>
                                    <td><?php echo isset($row->unit_name_kh)? $row->unit_name_kh:"";?></td>
                                    <td>
                                        ...
                                        <a href="#" data-json='<?php echo json_encode($row);?>' class="inline-button btn-delete" data-toggle="tooltip" title="Delete" url="<?php echo base_url();?>unit/delete"> <i class="fa fa-trash-o text-red"></i> </a>
                                    </td>
                                </tr>
                            <?php
                            }
...
//            }
        ...
        function post_search( page=1)
        {
            ...
            var url = "<?php echo base_url()?>unit/manage_unit";
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".btn-view", function(event){
            ...
            $("#dialog-unit .modal-title").html("View Unit");
            $("#unit-form").attr('action', '<?php echo base_url()?>unit/view_detail' );

            var model = JSON.parse($(this).attr('data-json'));
            $("#unit_id").val(model.unit_id);
            $("#unit_name").val(model.unit_name);
            $("#unit_name_kh").val(model.unit_name_kh);
        ...
        $(document).on("click",".btn-edit", function(event){
            ...
            $("#dialog-unit .modal-title").html("Edit Unit");
            $("#unit-form").attr('action', '<?php echo base_url()?>unit/edit' );
        ...
        $(document).on("click",".btn-delete", function(event){
            ...
            $(document).on("click", "#dialog-confirm #btn-ok", function(event){
                ...
                formData.append('unit_id', model.unit_id);
